...
<body>
    <div class="glass rounded-3xl p-8 max-w-sm w-full text-center">
        ...
        <div class="glass rounded-2xl px-4 py-4 mb-2 fade-up d3">
            <div class="flex items-center gap-3">
                <span class="text-white/60 text-xs tabular-nums w-10 text-right shrink-0" id="lblCurrent">0:00</span>
                <div class="progress-track" id="track">
                    <div class="progress-fill" id="fill"></div>
                </div>
                <span class="text-white/60 text-xs tabular-nums w-10 shrink-0" id="lblDuration">--:--</span>
            </div>
            <p class="text-white/30 text-xs mt-3" id="statusLabel">Loading...</p>
        </div>
    ...
    player.addEventListener('loadedmetadata', () => {
        if (isFinite(player.duration) && player.duration > 0) {
            lblDur.textContent = t(player.duration);
    ...
    player.addEventListener('timeupdate', () => {
        ...
        lblCur.textContent = t(player.currentTime);
        lblDur.textContent = t(player.duration);
    });
    ...
    document.getElementById('track').addEventListener('click', function(e) {
        if (!isFinite(player.duration) || player.duration <= 0) return;
        player.currentTime = ((e.clientX - this.getBoundingClientRect().left) / this.offsetWidth) * player.duration;
    });
    </script>
</body>
</html>